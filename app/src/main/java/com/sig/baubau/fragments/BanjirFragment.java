...
import com.sig.baubau.UI.Bencana;
...
public class BanjirFragment extends Fragment {
    ...
    Polygon wilayah = null;
    private static final List<PatternItem> PATTERN_ITEMS = Arrays.asList(DOT, DASH);
    ArrayList<Double> costs = new ArrayList<>();
    private DatabaseReference db;
    View viewLayout;
    TextView tKel, tKec, tKode, tAncaman;
    private modelBanjir model;
    ArrayList<Marker> markerList = new ArrayList<>();
    Marker markers;

    private ArrayList<modelBanjir> list;
    ...
    private OnMapReadyCallback callback = new OnMapReadyCallback() {
        ...
        public void onMapReady(GoogleMap gMap) {
            googleMap = gMap;
            showMaps();
            fab = viewLayout.findViewById(R.id.fabBanjir);
    ...
    private void showMaps() {
        ...
        ArrayList<modelBanjir> pusat1 = new ArrayList<>();
        ArrayList<modelBanjir> pusat2 = new ArrayList<>();
        ...
        db.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                pusat2.add(list.get(1));
                pusat2.add(list.get(20));
                pusat2.add(list.get(18));
                pusat2.add(list.get(7));

                pusat1.add(list.get(9));
                pusat1.add(list.get(15));
                pusat1.add(list.get(22));
                pusat1.add(list.get(28));
        ...
        ref.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                for (int i=0; i<latitude.size(); i++) {
                    ...
                    if (hasil.get(i).trim().equals("Tinggi")) {
                        options.fillColor(Color.parseColor("#40FF0000"));
                    }
                    ...
                    googleMap.setOnPolygonClickListener(new GoogleMap.OnPolygonClickListener() {
                        ...
                        public void onPolygonClick(@NonNull Polygon polygon) {
                            if (wilayah != null) {
                                wilayah.setStrokeWidth(0);
                                wilayah.setStrokePattern(null);
                            }
                            wilayah = polygon;
                            wilayah.setStrokeWidth(3);
                            wilayah.setStrokePattern(PATTERN_ITEMS);
                            String[] getTAG = wilayah.getTag().toString().split("\n");
        ...
        dbPosko.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                for (DataSnapshot ds : snapshot.getChildren()) {
                    ...
                    markers = googleMap.addMarker(new MarkerOptions().position(new LatLng(lat, lng))
    ...
    public View onCreateView(@NonNull LayoutInflater inflater,
        ...
        viewLayout = inflater.inflate(R.layout.fragment_banjir, container, false);
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        SupportMapFragment mapFragment =
                (SupportMapFragment) getChildFragmentManager().findFragmentById(R.id.mapBanjir);
        if (mapFragment != null) {
            mapFragment.getMapAsync(callback);
        }
    }

    private void posko(GoogleMap gMap) {
        DatabaseReference dbPosko = FirebaseDatabase.getInstance().getReference("posko");
        dbPosko.addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
    ...
    private ArrayList<String> manhattan(ArrayList<modelBanjir> pusatCentroid) {
        double[][] centroid = new double[pusatCentroid.size()][4];
        for (int i=0; i<pusatCentroid.size(); i++) {
            ...
            centroid[i][3] = pusatCentroid.get(i).getTinggi();
        }

        double[][] dataSampel = new double[list.size()][4];
        for (int i=0; i<list.size(); i++) {
            dataSampel[i][0] = list.get(i).getTidak();
            dataSampel[i][1] = list.get(i).getRendah();
            dataSampel[i][2] = list.get(i).getSedang();
            dataSampel[i][3] = list.get(i).getTinggi();
        }

        double[][] dist = distance(dataSampel, centroid);
        double[][] dis = new double[list.size()][4];
        ...
        for (int i=0; i<list.size(); i++) {
            double[] urut = new double[4];
            urut[0] = dis[i][0];
            urut[1] = dis[i][1];
            urut[2] = dis[i][2];
            urut[3] = dis[i][3];
            String[] an = {"Aman", "Rendah", "Sedang", "Tinggi"};
    ...
    private double[][] distance(double[][] data, double[][] cluster) {
        double[][] hasil = new double[data.length][4];
        for (int h=0; h<4; h++) {
