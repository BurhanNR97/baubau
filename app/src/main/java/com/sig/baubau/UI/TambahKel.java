...
public class TambahKel extends AppCompatActivity implements View.OnClickListener {
    String idku;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_tambah_kel);
        ...
        keluar = findViewById(R.id.addKel_keluar);
        simpan = findViewById(R.id.addKel_simpan);
        edKode = findViewById(R.id.addKel_kode);
        edNama = findViewById(R.id.addKel_nama);
        edLat = findViewById(R.id.addKel_Lat);
        edLng = findViewById(R.id.addKel_Lng);
        map = findViewById(R.id.addKel_map);
        ...
        map.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                Intent intent = new Intent(TambahKel.this, MapTambahKel.class);
                intent.putExtra("a", "1");
    ...
    public void onBackPressed() {
        ...
        Intent intent = new Intent(TambahKel.this, Kelurahan.class);
        intent.putExtra("id", idku);
    ...
    protected void onResume() {
        ...
        if (getIntent().getStringExtra("a").equals("0")) {
            DatabaseReference db = FirebaseDatabase.getInstance().getReference("kelurahan").child(idku);
            ...
            ref.addListenerForSingleValueEvent(new ValueEventListener() {
                ...
                public void onDataChange(@NonNull DataSnapshot snapshot) {
                    ...
                    if (a <= 0) {
                        edKode.setText(idku + "1001");
                    } else
                    if (a > 0) {
                        for (DataSnapshot ds : snapshot.getChildren()) {
                            String x = ds.getKey();
                            char[] y = x.toCharArray();
                            String z = x.substring(6, y.length);
                            int n = Integer.parseInt(z) + 1;
                            edKode.setText(idku + n);
                        }
                    }
                }
    ...
    public void onClick(View view) {
        ...
        if (kode.isEmpty()) {
            ...
            modelKel model = new modelKel();
            model.setKode(kode);
            model.setNama(nama);
            model.setLat(lat);
            model.setLng(lng);
            DatabaseReference db = FirebaseDatabase.getInstance().getReference("kelurahan").child(idku);
            DatabaseReference dbb = FirebaseDatabase.getInstance().getReference("kel");
            dbb.child(kode).setValue(model);
            db.child(kode).setValue(model).addOnCompleteListener(new OnCompleteListener<Void>() {
                @Override
                public void onComplete(@NonNull Task<Void> task) {
                    if (task.isSuccessful()) {
                        Toast.makeText(TambahKel.this, "Data berhasil ditambahkan", Toast.LENGTH_SHORT).show();
                        onBackPressed();
                    } else {
                        Toast.makeText(TambahKel.this, task.toString(), Toast.LENGTH_SHORT).show();
                    }
                }
            });
        }
